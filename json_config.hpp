...
class JsonConfig
{
  public:
    ...
    JsonConfig(std::function<void()> func) : _loadFunc(func)
    {
...
#ifdef USE_IBM_COMPATIBLE_SYSTEM
        auto propVals = getTargetProperties<std::vector<std::string>>(confCompatIntf, confCompatProp);
        if (!propVals.empty())
        {
       ...
       _prettyNameVals = getTargetProperties<std::string>(invItemIntf, prettyNameProp);
    ...
    void targetIntfAdded(sdbusplus::message_t& msg)
    {
        ...
        // valid folder name to look for configs. _propValInUse is
        // cleared when getConfFile() fails to find configs under this
        // folder name.
        if (!_propValInUse.empty())
        {
            return;
        }
  ...
  private:
    ...
    inline static std::string _propValInUse;
